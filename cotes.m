function Q=cotes(V,a,b,h,nodes)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %
    % cotes.m
    % beware Runge's phenomenon...
    %
    % 2 to 11 point Summed Newton-Cotes numerical integration formulas.
    %
    % Inputs: 
    %
    % V - vector of function evaluated at equidistant points
    % NOTE: currently feeling quite lazy so I am cheating a 
    % little bit and assuming I want to integrate as V(i,:)
    % a - lower limit of integration
    % b - upper limit of integration
    % h - spacing of V samples
    % nodes - number of wanted nodes in Newton-Cotes formula
    % 
    %
    % adapted to vector input (rather than function) from 8999 matlab files
    % original author information and matlab URL below:
    %
    % Written by: Greg von Winckel
    % Contact: gregvw(at)math(dot)unm(dot)edu
    % URL: http://math.unm.edu/~gregvw
    %
    % matlab URL: https://www.mathworks.com/matlabcentral/fileexchange/8999-summed-newton-cotes-rules
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    N = size(V, 1);
    
    N=(nodes-1)*floor(N/(nodes-1));  N1=N+1;
