function [X,s,k] = acosm(A)
    ...
    %   A must not have any real eigenvalues on the intervals (-inf,-1]
    %   or [1,inf), which are the branch cuts of the inverse cosine.
    %   [X,s,k] = acosm(A) also returns the number of matrix square roots
    %   computed and the degree k of the Pade approximant.
    %
    %   Reference:
    ...
    % https://github.com/aprahamian/matrix-inv-trig-hyp/blob/5b0e5a6308ec03e4c41e877734d0bf2dcc8d206e/acosm
    ...
    if any( imag(d) == 0 & (real(d) <= -1 | real(d) >= 1) )
        warning('Input has a real eigenvalue outside the interval (-1,1).')
        d'
    end
    ...
    while k == 0
        ...
        T = sqrtm((I+T)/2); % More scaling if needed
        s = s + 1;
        
    end
    
    % Pade coefficients obtained with Mathematica.
    % For better efficiency the Pade approximants should be valuated
